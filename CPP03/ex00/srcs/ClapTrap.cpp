...
void	ClapTrap::attack(const std::string& target)
{
	...
	{
		--this->energyPoints;
		LOG("Claptrap " << this->name << " attacks " << target
			<< " dealing " << this->attackDmg << "dmg");
	}
	else if (this->hitPoints <= 0)
		LOG("Claptrap is dead bruv");
	else
		LOG("Claptrap " << this->name << " is exhausted and cannot perform any actions");
}
...
void	ClapTrap::takeDamage( unsigned int amount )
{
	if (this->hitPoints > 0)
	{
		...
		LOG("Claptrap " << this->name << " took " << amount 
			<< "dmg and now has " << this->hitPoints << "HP");
		if (this->hitPoints <= 0)
			LOG("Claptrap " << this->name << " died" );
	}
	else
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	if (this->energyPoints > 0 && this->hitPoints > 0)
	{
		--this->energyPoints;
		this->hitPoints += amount;
		LOG("Claptrap " << this->name << " repairs itself for "
			<< amount << " and now has " << this->hitPoints << "HP");
	}
	else if (this->hitPoints <= 0)
		LOG("Claptrap " << this->name << " is dead bruv");
	else
		LOG("Claptrap " << this->name << " is exhausted and cannot perform any actions");
}
